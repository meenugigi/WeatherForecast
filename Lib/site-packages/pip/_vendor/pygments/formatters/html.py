...
class HtmlFormatter(Formatter):
    ...
    `noclasses`
        ...
        will not use CSS classes, but inline css. This is not recommended
    ...
    `cssstyles`
        Inline CSS css for the wrapping ``<div>`` tag (default: ``''``).

    `prestyles`
        Inline CSS css for the ``<pre>`` tag (default: ``''``).
    ...
    **Subclassing the HTML formatter**
    ...
    def _get_css_inline_styles(self, ttype):
        """Return the inline CSS css for this token type."""
    ...
    def _create_stylesheet(self):
        ...
        for ttype, ndef in self.style:
            ...
            if style:
                t2c[ttype] = name
                # save len(ttype) to enable ordering the css by
